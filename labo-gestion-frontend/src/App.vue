          ...
          <tr v-for="item in items" :key="item.id">
            ...
            <td class="actions-cell">
              <div class="use-action">
                <input type="number" v-model.number="decrementAmounts[item.id!]" min="1" class="small-input" />
...
<style scoped>
.container { max-width: 1100px; margin: 1.5rem auto; font-family: sans-serif; color: #333; }
.auth-header { display: flex; justify-content: space-between; align-items: center; padding: 12px; background: #eee; border-radius: 6px; margin-bottom: 2rem; }
...
.use-action, .manage-actions { display: flex; gap: 4px; }
.small-input { width: 50px; }

@media (prefers-color-scheme: dark) {
  .container { color: #ddd; }
  .auth-header { background: #222; }
  th { background: #222; }
  td { border-color: #444; }
  .form-section { background: #1a1a1a; border-color: #333; }
  input, select { background: #222; color: white; border-color: #444; }
}
</style>
